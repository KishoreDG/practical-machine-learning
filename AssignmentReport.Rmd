...
```{r, echo=TRUE}
# convert data  (character levels to numeric)
...
```{r, echo=TRUE}
# remove outcome from train data set
train$classe = NULL
```

Use data from accelerometers on the belt, forearm, arm, and dumbell, so the features are extracted based on these keywords.
...
system.time( bst <- xgboost(param=param, data=train.matrix, label=y,nrounds=min.merror.idx, verbose=0) )
```
